...
def calculate_histogram(image):
    hsv_image = np.array(rgb_to_hsv_vectorized(image))
    h_bins = 8
    s_bins = 8
...
def normalize_vector(image):
    norm = np.linalg.norm(np.array(calculate_histogram(image)))
    if norm == 0: 
        return calculate_histogram(image)
    return (calculate_histogram(image) / norm).tolist()

def calculate_cosine_similarity(vektor1, vektor2):
    similarity = np.dot(vektor1, vektor2)
